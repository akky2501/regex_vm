...
static inline void genMatch(void){
	code_top[getCodeCount()] = VM_Match;
	code_size++;
	opcode_size++;
}
...
RegexVMCode emitVMCode(RegexAST* ast){
	initGenCode();
	convertASTtoCode(ast,true);
	genMatch();
...
void printVMCode(vm_code_type* code){
	vm_addr_type PC = 0;
	while(code[PC] != VM_Match){
		printf("%04d : ",PC);
		switch(code[PC]){
			case VM_Char:
